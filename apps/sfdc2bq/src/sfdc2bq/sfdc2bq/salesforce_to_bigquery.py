...
class SalesforceToBigquery:
    ...
    def replicate(simple_sf_connection: Salesforce,
        ...
        logging.info(
            "Preparing Salesforce to BigQuery Replication: %s to %s.%s.",
            api_name, project_id, dataset_name)
        ...
        if not output_table_name:
            output_table_name = desc["name"]  # type: ignore

        logging.info("Target table: `%s.%s.%s`", project_id,
                     dataset_name, output_table_name)
        ...
        has_created_date = "createddate" in sfdc_field_names
        has_modified_date = "lastmodifieddate" in sfdc_field_names
        has_is_deleted = "isdeleted" in sfdc_field_names
        has_is_archived = "isarchived" in sfdc_field_names

        if not (has_system_mod_stamp or has_created_date or has_modified_date):
            logging.fatal("⛔️ %s is not supported (no SystemModstamp, LastModifiedDate or CreatedDate field).",
                          api_name)
            raise RuntimeError(
                f"{api_name} is not supported (no SystemModstamp, LastModifiedDate or CreatedDate field).")
        elif has_system_mod_stamp:
            mod_stamp_name = "SystemModstamp"
        elif has_modified_date:
            mod_stamp_name = "LastModifiedDate"
        else:
            mod_stamp_name = "CreatedDate"
        ...
        try:
            bq = BigQueryHelper(
                ...
                target_table_name=output_table_name,  # type: ignore
            ...
            query = SalesforceToBigquery._create_sfdc_query(
                api_name, ",".join(source_fields),
                recordstamp, mod_stamp_name,
    ...
    def _bulk_get_records(
        ...
        while job_running:
            ...
            status = sfdc_connection.restful(
                path=job_status_path, method="GET")
            state = status["state"]  # type: ignore
            if state in ["Failed", "Aborted"]:
                logging.fatal("⛔️ Operation %s %s: %s", job_id, state,
                              status["errorMessage"])  # type: ignore
                raise RuntimeError(
                    f"Operation {job_id} {state}: {status['errorMessage']}")  # type: ignore
    ...
    def _create_sfdc_query(
        ...
        query = (f"SELECT {column_list} FROM {api_name} "
                 f"WHERE {mod_stamp_name}<={recordstamp_str}")
        if last_record_stamp:
            ...
            last_record_stamp_str = last_record_stamp_minus_ten.strftime(
                "%Y-%m-%dT%H:%M:%S.000Z")
            query += f" AND {mod_stamp_name}>={last_record_stamp_str}"
